...
func TestTransactionsRollback(t *testing.T) {
	...
	if err := tx.Select().From("animals").Columns("COUNT(name)").LoadValue(ctx, &totalCount); err != nil {
		t.Fatal(err)
	...
	// Insert one record
	if _, err := tx.Insert().InTo("animals").Columns("name").Values("fuu").Exec(ctx); err != nil {
		t.Fatal(err)
	}
	...
	// if err := db.For(ctx).Select().From("animals").Columns("COUNT(name)").LoadValue(ctx, &totalCount); err != nil {
	// 	t.Fatal(err)
	// } else if totalCount != 0 {
	// 	t.Fatalf("Expected 0 record but got %d", totalCount)
	// }
	...
	if _, err := tx.Insert().InTo("animals").Columns("name").Values("bar").Exec(ctx); err != nil {
		t.Fatal(err)
	}
	...
	// Generate a conflict
	if _, err := tx.Update().Table("animals").Set("name", "fuu").Where("name = ?", "bar").Exec(ctx); err == nil {
	...
	if _, err := tx.Delete().From("animals").Where("name = ?", "bar").Exec(ctx); err == nil {
		t.Fatalf("Expected unique constraint to kick in when UPDATE but it did not")
